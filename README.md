# Jubiläums-Finder (Anniversary Finder)

This project is a web-based "Jubiläums-Finder" (Anniversary Finder) that allows users to calculate and explore special date and time milestones based on a given start date. These milestones can include various units like years, months, weeks, days, hours, minutes, and seconds, and can be filtered by different numerical patterns such as rounded multiples or repdigits. The application also provides the functionality to export selected milestones into an .ics calendar file.
...
*   **Interactive Filtering:** Filter results by unit type, pattern, and whether they are in the past or future.
*   **Pagination:** Load more future or past milestones in blocks of 10 years to manage large result sets.
...
2.  **Input Details:**
    ...
    *   **Startdatum:** Select the significant start date.
...
5.  **Calculate:** Click the "Jubiläen ausrechnen" (Calculate Anniversaries) button.
6.  **Explore Results:**
    *   The results will appear in a list.
    *   Use the "🔎 Filter anzeigen" button to show/hide and apply filters based on units, patterns, and time direction (Vergangenheit/Zukunft).
    *   Click "Weitere 10 Jahre laden" or "Vorherige 10 Jahre laden" to load more milestones.
...
*   **DOM Manipulation:** Interacting with form elements, result list, and filters.
*   **Date Calculations:** Custom functions for accurate date and time arithmetic.
*   **Pattern Matching:** Logic to identify specific numerical patterns for milestones.
*   **State Management:** Managing the application's state, including selected filters and loaded events.
*   **URL Parameters:** Encoding and decoding application state from the URL.
*   **ICS File Generation:** Creating valid `.ics` files for calendar export.
